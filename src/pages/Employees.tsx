...
const Employees: React.FC<EmployeesProps> = ({ previewOnly = false }) => {
  ...
  const [previewTagFilters, setPreviewTagFilters] = useState<
    PreviewTagFilter[]
  >([]);
  const [previewLogPage, setPreviewLogPage] = useState(0);
  const [previewLogRowsPerPage, setPreviewLogRowsPerPage] = useState(5);
  const [selectedConsoleEmp, setSelectedConsoleEmp] = useState<Employee | null>(
    null,
  );
  const [consoleFormData, setConsoleFormData] = useState<ConsoleFormData>({
    employee_code: "",
    name: "",
    deduction_absent: "",
    appraisal: "",
  ...
  const defaultProfileStart =
    new Date().getMonth() >= 3
      ? `${currentYear}-04-01`
      : `${currentYear - 1}-04-01`;
  const defaultProfileEnd =
    new Date().getMonth() >= 3
      ? `${currentYear + 1}-03-31`
      : `${currentYear}-03-31`;

  const [profileStartDate, setProfileStartDate] =
    useState<string>(defaultProfileStart);
  const [profileEndDate, setProfileEndDate] =
    useState<string>(defaultProfileEnd);
  const [profileTenureAnchorEl, setProfileTenureAnchorEl] =
    useState<null | HTMLElement>(null);
  const [profileViewMode, setProfileViewMode] = useState<"monthly" | "annual">(
    "monthly",
  );
  ...
  const [exportStartYear, setExportStartYear] = useState<number>(
    new Date().getFullYear() - 1,
  );
  const [exportEndYear, setExportEndYear] = useState<number>(
  ...
  useEffect(() => {
    if (selectedConsoleEmp) {
      setConsoleFormData({
        ...
        deduction_absent: selectedConsoleEmp.deduction_absent
          ? Number(selectedConsoleEmp.deduction_absent)
          : "",
        appraisal: selectedConsoleEmp.appraisal
          ? Number(selectedConsoleEmp.appraisal)
          : "",
        appraisal_effective_date:
          selectedConsoleEmp.appraisal_effective_date || "",
        leave_encashment: selectedConsoleEmp.leave_encashment
          ? Number(selectedConsoleEmp.leave_encashment)
          : "",
        late_arrival_deduction: selectedConsoleEmp.late_arrival_deduction
          ? Number(selectedConsoleEmp.late_arrival_deduction)
          : "",
        damages_recovery: selectedConsoleEmp.damages_recovery
          ? Number(selectedConsoleEmp.damages_recovery)
          : "",
        bonus_incentives: selectedConsoleEmp.bonus_incentives
          ? Number(selectedConsoleEmp.bonus_incentives)
          : "",
        other_deductions: selectedConsoleEmp.other_deductions
          ? Number(selectedConsoleEmp.other_deductions)
          : "",
  ...
  useEffect(() => {
    if (selectedConsoleEmp && employees.length > 0) {
      const latest = employees.find(
        (e: Employee) => e.id === selectedConsoleEmp.id,
      );
      if (
        latest &&
        JSON.stringify(latest) !== JSON.stringify(selectedConsoleEmp)
      ) {
  ...
  const saveConsoleMutation = useMutation(
    ...
    },
    {
      ...
      onError: (err: any) => {
        showToast(
          err.response?.data?.message || "Failed to update HR inputs",
          "error",
        );
      },
    },
    ...
    if (profileEmpId && employees.length > 0) {
      ...
      if (emp) {
        setProfileFormData({
          ...
          deduction_absent: emp.deduction_absent
            ? Number(emp.deduction_absent)
            : "",
          ...
          leave_encashment: emp.leave_encashment
            ? Number(emp.leave_encashment)
            : "",
          late_arrival_deduction: emp.late_arrival_deduction
            ? Number(emp.late_arrival_deduction)
            : "",
          damages_recovery: emp.damages_recovery
            ? Number(emp.damages_recovery)
            : "",
          bonus_incentives: emp.bonus_incentives
            ? Number(emp.bonus_incentives)
            : "",
          other_deductions: emp.other_deductions
            ? Number(emp.other_deductions)
            : "",
          remarks: emp.remarks || "",
        });
      }
  ...
  const computeEmployerPf = (monthlyCtc: string | number, pfDeduction: boolean) => {
    ...
    return Number(Math.min(ctc * 0.12, 1800).toFixed(2));
  };
  ...
  const computeEmployerEsi = (monthlyCtc: string | number) => {
    ...
    return Number((ctc * 0.0325).toFixed(2));
  };
  ...
  const handleProfileFormSubmit = (e: React.FormEvent) => {
    ...
    if (pfApplies && !uan) {
      showToast(
        "UAN is required when PF is applicable as Basic is below 15000",
        "error",
      );
      return;
    }
    ...
    const payload = {
      ...
      appraisal_effective_date:
        profileFormData.appraisal_effective_date || null,
    };
  ...
  const handleExportFinancials = async () => {
    ...
    try {
      ...
      showToast(
        err.response?.data?.message || "Failed to export financial summary",
        "error",
      );
    }
  };
    ...
    <Box>
          ...
          >
            <TextField
              ...
              InputProps={{
                startAdornment: (
                  <SearchIcon
                    sx={{ color: "var(--color-text-muted)", mr: 1 }}
                  />
                ),
              }}
              sx={{
      ...
      >
        ...
        <DialogContent sx={{ color: "var(--color-text-secondary)" }}>
          ...
          <strong>{employeeToArchive?.name}</strong>? They will be removed from
          this list and moved to the Archived section.
        </DialogContent>
      ...
      >
        ...
        <DialogContent sx={{ color: "var(--color-text-secondary)" }}>
          ...
          <strong>{employeeToChangeStatus?.name}</strong> as{" "}
          {employeeToChangeStatus?.active_status ? "Inactive" : "Active"}?
        </DialogContent>
        <DialogActions sx={{ p: 2.5, pt: 1.5 }}>
          <Button
            ...
            onClick={() => {
              if (employeeToChangeStatus) {
                updateMutation.mutate({
                  id: employeeToChangeStatus.id,
                  data: {
                    active_status: !employeeToChangeStatus.active_status,
                  },
                });
              }
